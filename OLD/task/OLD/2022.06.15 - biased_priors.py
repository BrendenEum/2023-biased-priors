...
nBlocks = 1 #how many blocks? There are breaks between blocks.
...
welcome_txt_stim = TextStim(
    ...
    text=
        ...
        displayed for a VERY short period of
        time (typically between 0.5 and 3 secs).
...
for ind, row in praccond_df.iterrows():
    ...
    while trial_clock.getTime() <= cross_time:
    ...
    while cross_time < trial_clock.getTime() <= cross_time+stim_time:
        
        fb_clock.reset()
        
        if not dots_generated:
            dot = Circle(
                win,
                edges = 128,
                radius = dotsize,
    ...
    while (cross_time+stim_time < trial_clock.getTime() <= cross_time+stim_time+choice_time) and press_rt == None: 
    ...
    while press_name == '' and cross_time+stim_time+choice_time < trial_clock.getTime() and fb_clock.getTime() <= fb_time:
    ...
    while press_name != '' and fb_clock.getTime() <= fb_time:
...
for block in range(nBlocks):
    if int(paydf['difficulty'][block])>50:
        ...
        correct = 'left'
    if paydf['response'][block]==correct:
